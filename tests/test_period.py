...
def test_unsupported_period_raises():
    with pytest.raises(ValueError):
        resolve_period("next-month", date(2026, 9, 8))
...
def test_next_day_resolves_to_the_day_after_reference():
    assert resolve_period("next-day", date(2026, 9, 8)) == {"Wed": date(2026, 9, 9)}


def test_next_day_rolls_over_the_week_boundary():
    assert resolve_period("next-day", date(2026, 9, 13)) == {"Mon": date(2026, 9, 14)}
...
def test_next_week_always_resolves_to_the_following_monday_to_sunday(reference: date):
    dates = resolve_period("next-week", reference)

    assert dates["Mon"] == date(2026, 9, 14)
    assert dates["Sun"] == date(2026, 9, 20)
